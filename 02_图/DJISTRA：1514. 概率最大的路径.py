...
class Solution:
    def maxProbability(self, n: int, edges: List[List[int]], succProb: List[float], start: int, end: int) -> float:
        ...
        dist = {}
        while priorityq:
            p, node = heappop(priorityq)
            if node in dist: continue  # 更新过的不再更新，因为DISJKSTRA算法要求不能有负权值，所以贪心一直正确
            dist[node] = -p
            if node == end:
                return dist[node]  # 提前返回
            for nei, w in graph[node]:
                if nei not in dist:
                    heapq.heappush(priorityq, (p * w, nei))
        return 0

        # dijkstra算法 就是一种BFS罢了
        ...
        # 需要构造一个优先队列来辅助实现
        queue = []
        heapq.heappush(queue, (-1, start))  # 大顶堆  负数
        dist[start] = 1
        while (queue):
